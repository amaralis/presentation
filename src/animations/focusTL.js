...
export default function focusBody(camera, body, {focusShader}, scene, renderer, nextBody){
    ...
    tl.to(camera.userData.camTargetObj.position, {x: body.position.x, y: body.position.y, z: body.position.z, duration: 1, ease: "power2.inOut", onStart: function(){
        ...
        // Push all the children of current camera target body into its orbital system array, so we can find out if other bodies are its moons
    ...
    onComplete: function(){
        // console.log("cam target pos arrived at", body.name, camera.userData.camTargetObj.position)
    ...
    .to(camera, {fov:90, duration: 1, ease: "back.inOut(1)", delay: 4,
        ...
        onComplete: function(){
            ...
            // console.log("position at animation complete:", camera.userData.camera.userData.camTargetObj.position);
            ...
            // gsap.to(camera.userData.camTargetObj.position, {x: primordial1.position.x, y: primordial1.position.y, z: primordial1.position.z, duration: 2, delay: 2, 
            //     onComplete: function(){
            //         // camera.userData.removeCamTargetFromOrbit();

            //         camera.userData.insertCamTargetIntoOrbit(volcanic1.parent);
            //         gsap.to(camera.userData.camTargetObj.position, {x: volcanic1.position.x, y: volcanic1.position.y, z: volcanic1.position.z, duration: 2, delay: 6})
            //     }
            // })
